...
function formatLanguageLabel(language) {
  ...
  return String(language).replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
}
...
const CodeBlock = ({ code, language, title = 'Show Code' }) => {
  ...
  const langLabel = formatLanguageLabel(language);
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-sm text-slate-600 dark:text-slate-300">
              {isOpen ? 'Syntax-highlighted snippet' : title}
            </p>
          </div>
        </div>
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            ...
            >
              <pre className="mb-0 overflow-x-auto px-4 py-4 md:px-5 md:py-5">
